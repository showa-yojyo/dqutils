...
class StateMachineTestCase(AbstractStateMachineTestCase):
    ...
    def test_run_near_boundary_opcode(self):
        ...
        self.assertRegex(output_lines[-2], r"^CA/FFFF:\s+FF$")  # !!
        self.assertEqual(output_lines[-1], "")
    ...
    def test_run_near_boundary_opcode(self):
        ...
        self.assertRegex(output_lines[-2], r"^CE/FFFF:\s+6B\s+RTL$")
        self.assertEqual(output_lines[-1], "")
    ...
    def test_run_until_return(self):
        ...
        self._do_test_until_option(0xC2B091, r"^C2/B099:\s+60\s+RTS$")
        self._do_test_until_option(0xC2B4AF, r"^C2/B501:\s+6B\s+RTL$")
    ...
    def test_run_brk_operand(self):
        ...
        for i in (
            ...
            line = output_lines[i]
            self.assertRegex(line, r"^C3/E6[0-9A-F]{2}:\s+00[0-9A-F]{2}07\s+")
            self.assertRegex(line, r"BRK #\$07[0-9A-F]{2}$")

        self.assertRegex(output_lines[-2], r"^C3/E62F:\s+6B\s+RTL$")
        self.assertEqual(output_lines[-1], "")
